...
func (slct *Select) keyResponse() xevent.KeyPressFun {
	f := func(X *xgbutil.XUtil, ev xevent.KeyPressEvent) {
		...
		case keybind.KeyMatch(X, slct.config.ConfirmKey, mods, kc):
			slct.data = mods&xproto.ModMaskShift > 0
			...
			if slct.selected >= 0 && slct.selected < len(slct.items) {
				...
				slct.items[0].choose()
				slct.Hide()
			} else if slct.config.ConfirmFn != nil {
				slct.config.ConfirmFn(string(slct.Input.Text))
				slct.Hide()
			}
			return
...
func (slct *Select) FilterItems(search string) {
	...
	for _, group := range slct.groups {
		...
		for _, item := range group.items {
			...
			case TabCompleteCustom:
				if !slct.config.CompleteFn(search, item.text) {
					item.hide()
					continue
				}
			default:
...
type SelectConfig struct {
	...
	ConfirmKey   string
	TabKey       string
	CompleteFn   func(input, item string) bool
	ConfirmFn    func(cmdline string)
}
